...
- Commands:
  ...
  - `TE-Copilot: Send Selection` (teCopilot.sendSelection) — adds the current selection to the chat context and notifies in the chat panel
  - `TE-Copilot: Set OpenAI API Key` (teCopilot.setOpenAIKey) — stores your key securely in Secret Storage
...
5. In the Activity Bar, click on the "TE-Copilot" icon to open the Chat view.  
   Alternatively, run the command: `TE-Copilot: Open Chat`.
...
- Secret: `teCopilot.openai.apiKey` is stored in VS Code Secret Storage when running `TE-Copilot: Set OpenAI API Key`.
...
- Previous settings namespaces `teCopilo.*` and `kaviaChat.*` continue to be read as fallbacks.
