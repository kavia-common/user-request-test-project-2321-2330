...
export async function mcpReadFile(filePath?: string): Promise<string | undefined> {
  try {
    ...
    if (!abs) {
      vscode.window.showWarningMessage('TE-Copilot: No file path provided or not within workspace.');
      return;
    }
    ...
    vscode.window.showErrorMessage(`TE-Copilot: Read failed - ${e?.message || e}`);
    return;
  }
}
...
export async function mcpWriteFile(filePath?: string, content?: string, options?: { append?: boolean }): Promise<boolean> {
  try {
    ...
    const ok = await vscode.window.showWarningMessage(
      `TE-Copilot: Confirm ${options?.append ? 'append to' : 'write'} file?\n${abs}`,
      { modal: true },
      'Yes'
    );
    ...
    vscode.window.showErrorMessage(`TE-Copilot: Write failed - ${e?.message || e}`);
    return false;
  }
}
...
export async function mcpProduceDiff(filePath?: string): Promise<string | undefined> {
  ...
  const header = `// Updated by TE-Copilot at ${new Date().toISOString()}\n`;
...
export async function mcpDiffApply(filePath?: string, diff?: string): Promise<boolean> {
  ...
  const ok = await vscode.window.showWarningMessage(
    `TE-Copilot: Apply proposed diff to ${abs}?`,
    { modal: true },
    'Apply'
  );
